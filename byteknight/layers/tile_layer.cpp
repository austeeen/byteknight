...
TileLayer::TileLayer(const std::string& name, bt::SceneContext* scene_context, const LayerAsset &lyr_ast) :
LayerBase(name, scene_context, lyr_ast), scenedata(scene_context->getSceneData())
{
    bt::warnForBadValue<int>(lyr_ast.lyr_cols, GETNAME(lyr_ast.lyr_cols), "TileLayer");
};
TileLayer::~TileLayer()
{
...
void TileLayer::__generateTiles()
{
    for (unsigned int i = 0; i < scenedata.total_tiles; i++) {
        ...
        bt::coord c(i, lyr_ast.lyr_cols);
        bt::rect r(c, (sf::Vector2f) scenedata.tile_size.size);
